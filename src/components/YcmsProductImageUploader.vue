  ...
  export default {
    ...
    methods: {
      ...
      _xhr: function (formData, keys, callback) {
        ...
        post(this.url, formData, config).then((response) => {
          keys.forEach((key) => {
            this.$set(this.files[key], 'uploaded', true);

            this.total++;
          });
          this.images_ = _.cloneDeep(response.images)
          th.depthFirstSearch(this.images_, (childNode) => {
            childNode.droppable = false;
          })
